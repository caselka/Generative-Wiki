...
export async function generateAsciiArt(topic: string): Promise<AsciiArtData> {
  ...
  const artPrompt = `You are an expert ASCII artist. Your task is to create a conceptual diagram for the topic: "${topic}".
The diagram must be a hierarchical tree, starting with "${topic}" at the top.

**RULES:**
1.  **USE ONLY THESE CHARACTERS:**
    - Box drawing: │ ─ ┌ ┐ └ ┘ ├ ┬ ┴
    - Spaces for alignment.
    - Text inside boxes can be uppercase letters, numbers, and '|' as a separator.
2.  **PERFECT ALIGNMENT:** The diagram MUST be perfectly aligned for a monospace font. All vertical lines must connect seamlessly.
3.  **JSON OUTPUT:** Respond with ONLY a single raw JSON object. The object must have one key, "art", whose value is a single string containing the entire diagram. Use \\n for newlines within the string.

**EXAMPLE for the topic "LANGUAGE":**
{
  "art": "        ┌──────────┐\\n        │ LANGUAGE │\\n        └────┬─────┘\\n             │\\n    ┌────────┼────────┐\\n    │        │        │\\n┌───────┐┌───────────┐\\n│ GRAMMAR ││ VOCABULARY│\\n└────┬────┘└─────┬─────┘\\n     │            │\\n┌────────┐  ┌──────────┬──────────┬───────────┐\\n│ SYNTAX │  │ MORPHOLOGY │ SEMANTICS│ PRAGMATICS│\\n└────────┘  └──────────┴──────────┴───────────┘"
}

Now, create the JSON object for the topic: "${topic}".`;
